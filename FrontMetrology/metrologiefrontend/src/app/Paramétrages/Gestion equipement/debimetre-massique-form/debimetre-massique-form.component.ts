...
@Component({
  selector: 'app-debimetre-massique-form',
  templateUrl: './debimetre-massique-form.component.html',
  styleUrls: ['./debimetre-massique-form.component.css']
})
export class DebimetreMassiqueFormComponent implements OnInit {
  ...
  OnSubmit() {
    ...
    if (this.userService.Method == "Post") {
      ...
      this.equipementservice.formEquipement.value.libelle = "DebimetreMassique"
      console.log(this.equipementservice.formEquipement)
      this.equipementservice.PostDebimetreMassique().subscribe(res => {
      ...
      this.equipementservice.PuttDebimetreMassique().subscribe(res => {
        if (res as Equipement) {
          this.bsModalRef.hide();
