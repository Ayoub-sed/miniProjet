...
@Component({
  selector: 'app-etuve-form',
  templateUrl: './etuve-form.component.html',
  styleUrls: ['./etuve-form.component.css']
})
export class EtuveFormComponent implements OnInit {
  ...
  OnSubmit() {
    ...
    if (this.userService.Method == "Post") {
      ...
      this.equipementservice.formEquipement.value.libelle = "Etuve"
      console.log(this.equipementservice.formEquipement)
      this.equipementservice.PostEtuve().subscribe(res => {
      ...
      this.equipementservice.PuttEtuve().subscribe(res => {
        if (res as Equipement) {
          this.bsModalRef.hide();
