...
@Component({
  selector: 'app-thermohygometre-form',
  templateUrl: './thermohygometre-form.component.html',
  styleUrls: ['./thermohygometre-form.component.css']
})
export class ThermohygometreFormComponent implements OnInit {
  dateE: string;
  dateV: string;
  ...
  OnSubmit() {
    ...
    if (this.userService.Method == "Post") {
      ...
      this.equipementservice.formEquipement.value.libelle = "Thermohygometre"
      console.log(this.equipementservice.formEquipement)
      this.equipementservice.PostThermohygometre().subscribe(res => {
      ...
      this.equipementservice.PuttThermohygometre().subscribe(res => {
        if (res as Equipement) {
          this.bsModalRef.hide();
