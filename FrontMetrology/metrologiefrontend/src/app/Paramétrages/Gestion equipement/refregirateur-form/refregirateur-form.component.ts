...
@Component({
  selector: 'app-refregirateur-form',
  templateUrl: './refregirateur-form.component.html',
  styleUrls: ['./refregirateur-form.component.css']
})
export class RefregirateurFormComponent implements OnInit {
  ...
  OnSubmit() {
    ...
    if (this.userService.Method == "Post") {
      ...
      this.equipementservice.formEquipement.value.libelle = "Refrigerateur"
      console.log(this.equipementservice.formEquipement)
      this.equipementservice.PostRefrigerateur().subscribe(res => {
      ...
      this.equipementservice.PuttRefrigerateur().subscribe(res => {
        if (res as Equipement) {
          this.bsModalRef.hide();
