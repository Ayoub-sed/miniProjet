...
@Component({
  selector: 'app-four-dorm',
  templateUrl: './four-dorm.component.html',
  styleUrls: ['./four-dorm.component.css']
})
export class FourDormComponent implements OnInit {
  ...
  OnSubmit() {
    ...
    if (this.userService.Method == "Post") {
      ...
      this.equipementservice.formEquipement.value.libelle = "Four"
      console.log(this.equipementservice.formEquipement)
      this.equipementservice.PostFour().subscribe(res => {
      ...
      this.equipementservice.PuttFour().subscribe(res => {
        if (res as Equipement) {
          this.bsModalRef.hide();
